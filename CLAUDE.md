...
- **Fix suggestions** (`FixSuggestion` / `FixSuggestionLine`, `/admin/suggestions`):
  ...
  - `FixSuggestion#line_states` reports, for each line, whether it is already in the guessed game (imported, with matching or different values), added by applying, or still to add. A suggestion whose every line is already there with the same values has nothing to fix; only Scorebook's player page is wrong. `classify!` moves such a suggestion from `pending` to **`not_needed` (対応不要)**, and back to `pending` if a missing line turns up. The job reclassifies at the start of every run and whenever it adds lines. It never touches approved or discarded suggestions.
  - Sometimes the player's Scorebook page also has a line on the same day filed under the player's own university's game (`FixSuggestion.twin_of`, stored as `twin_scorebook_game_id`). In that case the misfiled line isn't a missing line: it's a box score from some other game that has strayed in. Such lines are never applied. When every line of a suggestion is like this (`stray?`), it is shown as 「別の試合の成績の紛れ込み（要確認）」.
